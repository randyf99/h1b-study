...
class Description extends Component {
    ...
    get previousYearFragment() {
        ...
        if (year === '*') {
            ...
            if (this.props.data.length / lastYear.length > 2) {
                fragment =
                    ', ' + (this.props.data.length / lastYear.length).toFixed() + ' times more than the year before';
            } else {
    ...
    get countyFragment() {
        ...
        let ordered = _.sortBy(
            _.keys(byCounty).map(county => byCounty[county]).filter(d => d.length / this.props.data.length > 0.01),
        ...
        ordered = _.sortBy(
            _.keys(byCity).map(city => byCity[city]).filter(d => d.length / best.length > 0.01),
        ...
        return (
            <span>
                The best city {jobFragment.length ? `for ${jobFragment} on an H1B` : 'for an H1B'}{' '}
                {this.yearFragment ? 'was' : 'is'} <b>{city}</b> with an average salary ${this.format(mean - countyMedian)}{' '}
                above the local household median. Median household income is a good proxy for cost of living in an area.{' '}
    ...
    render() {
        ...
        return (
            <p className="lead">
                {this.yearsFragment ? this.yearsFragment : 'Since 2012'} the {this.UStateFragment} tech industry{' '}
